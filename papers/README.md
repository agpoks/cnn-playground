...
eleven is in [`references.bib`](references.bib).
...
## Why these eleven, and the ladder they form

Ten of these are genuine CNNs, each isolating one specific architectural
...
block, added on top of any of the above) -> **U-Net** (the same
convolution/pooling vocabulary, repurposed as an encoder-decoder with skip
connections for *dense* per-pixel prediction instead of one label per
image).
